...
var dimension = [canvas.width /2, canvas.height / 1.9];
...
function drawClock(place) {
  ...
  // Draw the logo.
  const img = new Image();
  img.src = "./bluebezel.png";
  img.decode().then(() => {
    // Translate the center of the logo
    // to the center of the canvas.
    let size = imgSize(img.width, img.height, 2.9 * clockRadius);
    var coord = translate(scale(size, [-1 / 2, -1 / 2]), dimension);
    context.drawImage(img, coord.x, coord.y, size[0], size[1]);

    const logo = new Image();
    logo.src = "./manCityLogo.png";
    logo.decode().then(() => {
      // Translate the center of the logo
      // to the center of the canvas.
